...
if($NOTA)
{
    ...
    $Receptor=mysqli_query($conexion,"SELECT DISTINCT u.id_usuario AS id, u.nombre as nombre , u.apellidos as apellidos , u.correo as correo FROM usuario u INNER JOIN notas n on u.id_usuario=n.Comun where n.Comun=$Comun LIMIT 0,1");
    if ($Receptor && $Emisor) 
    {
          $Electronico=mysqli_fetch_array($Emisor);
            $NombreCompleto=$Electronico['nombre']." " . $Electronico['apellidos'];
            $CorreoEnvia=$Electronico['correoo'];
        ...
        //Recipients
            $mail->setFrom("$zc",$NombreCompleto);
