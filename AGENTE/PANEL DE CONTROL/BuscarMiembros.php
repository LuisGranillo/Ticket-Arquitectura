...
<head>
    ...
    <title>Buscar miembros de equipo</title>
...
$Equipo="SELECT e.nombre , e.descripcion, e.idEquipo , u.id_Usuario FROM  usuario u INNER JOIN equipo e on u.id_usuario = e.idUsuario INNER JOIN trabaja t on t.idEquipo=e.idEquipo WHERE t.id_usuario=$n AND e.idEquipo!=894";  
...
<button placeholder="Buscar"class="btn btn-light" type="submit"><span class="glyphicon glyphicon-search"></span></button>
                ...
                <table style="width:100%;"class="tabla">  
                        ...
                        $WWW=" SELECT equipo.nombre as Name, usuario.correo,usuario.nombre,usuario.apellidos from usuario inner JOIN trabaja t on usuario.id_usuario = t.id_usuario inner join equipo on equipo.idEquipo=t.idEquipo WHERE t.idEquipo=$Miembro";
                        $WWW2 = mysqli_query($conexion,$WWW);
                       if ($WWW2) {
                           $WWW3= mysqli_fetch_array($WWW2);
                           if($WWW3!=null)
                           { echo "Miembros de " .$WWW3['Name'];
                           }    echo "</h3>";
                       }
                       ?>
                          <style>
...
text-overflow: ellipsis;
                        ...
                        $WWW="SELECT DISTINCT e.nombre as Name, u.correo,u.nombre,u.apellidos,es.tipo FROM usuario u INNER JOIN trabaja t on u.id_usuario = t.id_usuario INNER JOIN  equipo e on e.idEquipo=t.idEquipo  inner join estatus  es on u.id_estatus=es.idEstatus WHERE t.idEquipo=$Miembro";
                        $WWW2 = mysqli_query($conexion,$WWW);
                   
                         if ($WWW2) {
                             while ($WWW3=mysqli_fetch_array($WWW2)) {
                                 echo "<tr>";
                                 echo "<td ><input type='checkbox'></td>";
                                 echo "<td>" .  $WWW3['nombre'] . " " . $WWW3['apellidos']. "</td>";
                                 echo "<td>" .  $WWW3['correo'] ."</td>";
                                 echo "<td>" .  $WWW3['tipo'] ."</td>";
