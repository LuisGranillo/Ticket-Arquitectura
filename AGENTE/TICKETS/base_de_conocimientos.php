...
$respuestas = "SELECT Respuesta,id_pregunta from pregunta";
$preguntas = "SELECT pregunta,id_pregunta, id_categ from pregunta";
$temas = "SELECT tema,id_categ from categoria";
...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div id="page-inner">
            <h1 class="page-head-line">Base de Conocimientos <i class="fa fa-clipboard" aria-hidden="true"></i></h1>          
                <section class="cd-faq">        
                    <ul class="cd-faq-group" id="basics">
                        <?php $respuesta_t = mysqli_query($conexion, $temas);
                            while($row1=mysqli_fetch_row($respuesta_t)) {?>
                            <li class="cd-faq-title">
                                <h2><?php echo $row1[0] ?></h2>
                            </li>
                            <?php $respuesta_p = mysqli_query($conexion, $preguntas);
                                while($row2=mysqli_fetch_row($respuesta_p)) {
                                if($row2[2] == $row1[1])
                                {?>
                                    <li>
                                     <a class="cd-faq-trigger" href="#0"><?php echo $row2[0] ?></a>
                                        <div class="cd-faq-content">
                                            <?php $respuesta_r = mysqli_query($conexion, $respuestas);
                                                while($row3=mysqli_fetch_row($respuesta_r)) {
                                                    if($row3[1] == $row2[1])
                                                    {?>
                                                    <p><?php echo $row3[0] ?></p>
                                            <?php }} mysqli_free_result($respuesta_r)?>
                                    </div>
                                </li>
                            <?php }} mysqli_free_result($respuesta_p)?>
                            <br>
                        <?php } mysqli_free_result($respuesta_t)?>                        
                        <a href="nueva.php?id=<?php echo $id?>" class="btn btn-success">Agregar Categoria</a>
                        <a href="nueva_pregunta.php?id=<?php echo $id?>" class="btn btn-success">Agregar Pregunta</a>
                    </ul>
