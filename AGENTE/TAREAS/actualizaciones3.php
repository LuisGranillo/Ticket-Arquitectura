...
$consulta_usuario="SELECT*FROM  usuario WHERE id_usuario='$procurador'";
$results=$conexion->query($consulta_usuario);
while($cadena=$results->fetch_assoc()){

$nom_prc=$cadena['nombre']; 
$ap_pr=$cadena['apellidos'];
$nombre_usuario_responsable=''.$cadena['nombre'].'\n'.$cadena['apellidos'].'';
}
include("conexion1.php");



$query4="SELECT*FROM  aclaraciones WHERE id_usuario='$id_usuario'";
...
$id_aclaracion=$row4['id_aclaracion'];
  ...
  $codigo=$rowdepart['id_usuario'];
    ...
    $query1="UPDATE tarea SET procurador='$usrw',pr='$id_proc' where idTarea='$id_tarea'";
    ...
    $query6="INSERT INTO aclaraciones (id_usuario,Motivo,idTarea) VALUES ('$user','$aclaracion','$id_tarea')";
    ...
    try {
      ...
      $ap=$rowact['apellidos'];
        ...
        $correos="SELECT*FROM usuario WHERE id_usuario=$id_proc";
      ...
      $apellido_usuario2=$rowactc['apellidos'];
        ...
        $mail->Subject = " Hola agente  se ha asingado a : $nombre_usuario2 $apellido_usuario2 como responsable de la tarea";//Asunto
        $mail->Body    = "<p style='text-align: justify;'> Hola agente se ha cambiado el responsable de la tarea a : $nom_prc $ap_pr , la tarea modificado tiene como titulo : $Titulo agradecemos tu responsabilidad de la tarea: $id_tarea,
     y la razón por la que se cambio el departamento es por lo siguiente: $aclaracion .Favor de estar atento en su vencimiento y verificar dentro de la plataforma gracias.</p></b>
        <br>
        <br>
        ...
        echo "<script> 
            alert('Se  ha modificado el procurador de la tarea de manera correcta, ahora el responsable es: $nom_prc $ap_pr'); 
            window.location.href='/UTP/AGENTE/TAREAS/detalles2.php?idTarea=$id_tarea&codigo=$codigo&id_cola=$id&usur=$codigo'; 
            </script>";
