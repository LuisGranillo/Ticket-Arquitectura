...
if ($tareas_asignadas_por_equipo) {
    while ($restt = mysqli_fetch_array($tareas_asignadas_por_equipo)) {
       ...
       <?php                                     
            ...
            $colaborador_tarea = $mysqli -> query ("SELECT *FROM colaborador WHERE id_colab='$n'");
...
<div class="search-box">
                      ...
                      <img src="../imagenes/b.png" style="border raidus: 5px;"width="20" height="20" />
                            ...
                            <?php echo "<a  href='../TAREAS/new.php?codigo=$n'>"?><i class="fa fa-arrow-down fa-lg"></i>Creado más reciente</a>
                              <?php echo "<a  href='../TAREAS/time_tarea.php?codigo=$n'>"?><i class='dropdown-item'></i>Fecha de Vencimiento</a>

                              <?php echo "<a  href='../TAREAS/tareas.php?codigo=$n'>"?><i class='dropdown-item'></i>Tarea Número</a>
                    ...
                    <th >Asignado a:</th>
                                         ...
                                         <?php
                                             ...
                                             include("conexion3.php");
                                              $fecha_de_ahora = $mysqli -> query ("SELECT * FROM tarea  WHERE CURRENT_DATE() = date(Creacion_tarea)");
                                              if ($fecha_de_ahora) {
                                                  while ($fechas = mysqli_fetch_array($fecha_de_ahora)) {
                                                      $n=$_GET['codigo'];
                                                      if ($fechas['id_usuario']==$n) {
                                                          $actual=$fechas['Creacion_tarea'];
                                                          $usera=$fechas['Creacion_tarea'];
                                                          $tareas_de_usuario_logeado = $mysqli -> query("SELECT*FROM tarea WHERE id_usuario='$usera' OR Creacion_tarea='$actual'");
                                                          if ($tareas_de_usuario_logeado) {
                                                              while ($valores2 = mysqli_fetch_array($tareas_de_usuario_logeado)) {
