...
<body>
  ...
  <br>
                              ...
                              <table id="tabla" class="table table-dark">
                                  ...
                                  <tbody>
                                  <?php
                          $ESTATUS = mysqli_query($conexion,"SELECT DISTINCT usuario.nombre as Usuario,equipo.nombre  as Equipo,trabaja.id_trabaja as trabaja , usuario.apellidos from usuario INNER JOIN trabaja on usuario.id_usuario=trabaja.id_usuario INNER JOIN equipo ON trabaja.idEquipo= equipo.idEquipo WHERE trabaja.idEquipo!=894 and (
                            usuario.nombre LIKE '$Busqueda' 
                        or  equipo.nombre LIKE '%$Busqueda%' 
                        or usuario.id_usuario LIKE '%$Busqueda%'
                         or usuario.apellidos Like '%$Busqueda%'
                        or equipo.descripcion Like '%$Busqueda%') ORDER BY equipo.nombre DESC");
                           if ($ESTATUS) {
                               while ($row=mysqli_fetch_array($ESTATUS)) {
                                      ...
                                      <?php echo "<td> ". $row['Equipo'] . "</td>";?>
                                  
                                      <?php echo "<td> ". $row['Usuario'] . " " .$row['apellidos']. "</td>";?>
